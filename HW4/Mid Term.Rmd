---
title: "Mid Term Machine Learning: K Means Clustering"
...
```{r}
#The optimal choice was 5 clearly 15 is too many clusters as can be seen from the garphs of the spread of the clusters themselves
```
...
Tufts <- filter(university_all, College.Name == "Tufts University")
norm <- preProcess(Tufts[, -c(1, 2, 3, 10)], method=c("center", "scale"))
Tufts[, -c(1, 2, 3, 10)] <- predict(norm, Tufts[, -c(1, 2, 3, 10)])
...
```{r}
#Impute NAs with average from cluster 3
cluster_3 <- filter(Comparison, clusters_index_3 == 3)
avg_cluster_3 <- mean(cluster_3[,c(10)])
Tufts[, c(10)] <- avg_cluster_3
Tufts[, c(10)]
```
